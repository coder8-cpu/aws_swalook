...
class billing_serializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        validated_data['week'] = self.get_week_number(mon.day)
        ...
        validated_data.pop('json_data', None)
    ...
    def create(self,validated_data):
        ...
        for i in range(1, 13):
            s = StaffSetting(
                ...
                # staff_slab = validated_data["staff_slab"],
                # staff_target_business = validated_data["staff_target_business"],
                # staff_commision_cap = validated_data["staff_commision_cap"],
        ...
        if int(validated_data['grand_total']) > 100:
            ...
            try:
                ...
                if validated_data['loyality_points_deducted'] != "":
                    clp_object.current_customer_points = clp_object.current_customer_points - int(validated_data['loyality_points_deducted'])
                    clp_object.current_customer_points = clp_object.current_customer_points + int(validated_data['loyality_points'])
                    clp_object.save()
                    ledger_object = VendorCustomerLoyalityLedger()

                    ledger_object.vendor_branch_id = validated_data['vendor_branch_id']
                    ledger_object.user = self.context.get('request').user
                    ledger_object.customer = clp_object
                    ledger_object.point_spend  = validated_data['loyality_points_deducted']
                    ledger_object.point_available = clp_object.current_customer_points
                    ledger_object.inventory_invoice_obj = validated_data['slno']
                    ledger_object.invoice_obj = ""
                    ledger_object.date = dt.date.today()
                    ledger_object.month = mon.month
                    ledger_object.year = mon.year
                    ledger_object.week = validated_data["week"]
                    ledger_object.save()
...
class VendorCustomerLoyalityProfileSerializer(serializers.ModelSerializer):
    ...
    def create(self,validated_data):
        # try:
        #     customer = VendorCustomers.objects.get(mobile_no=validated_data['mobile_no'],vendor_branch=vendor_branch_,user=self.context.get('request').user)
        #     return "already exists customer profile with this membership"
        # except Exception:
...
class VendorLoyalityTypeSerializer(serializers.Serializer):
        ...
        #     model = VendorLoyalityProgramTypes
        #     fields = ["id","vendor_branch_name","program_type","expiry_duration","points_hold","price"]
        #     extra_kwargs = {'id':{'read_only':True},}
